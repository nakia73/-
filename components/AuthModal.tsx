  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: window.location.origin, // 認証後にアプリに戻るURL
          queryParams: {
            access_type: 'offline',
            prompt: 'consent',
          },
      ...
      addToast(err.message || "Google login failed. Check console for details.", 'error');
    }
  };
      ...
      <div className="bg-surface border border-white/10 rounded-2xl p-8 max-w-md w-full shadow-2xl animate-slide-in">
        ...
        <div className="mb-6">
          <button
            ...
            className="w-full py-3 bg-white text-black font-bold rounded-xl hover:bg-gray-200 transition-colors flex items-center justify-center gap-3"
          >
